...
func testAuthorToManyBookAuthors(t *testing.T) {
	...
	for _, v := range check {
		if v.AuthorsID == b.AuthorsID {
			bFound = true
		}
		if v.AuthorsID == c.AuthorsID {
			cFound = true
		}
	}
...
func testAuthorToManyAddOpBookAuthors(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.AuthorsID {
			t.Error("foreign key was wrong value", a.ID, first.AuthorsID)
		}
		if a.ID != second.AuthorsID {
			t.Error("foreign key was wrong value", a.ID, second.AuthorsID)
		}
...
var (
	authorDBTypes = map[string]string{`ID`: `bigint`, `Name`: `character varying`}
	_             = bytes.MinRead
)
