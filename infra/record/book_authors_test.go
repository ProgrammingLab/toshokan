...
func testBookAuthorToOneSetOpBookUsingBook(t *testing.T) {
	...
	for i, x := range []*Book{&b, &c} {
		...
		if a.BooksID != x.ID {
			t.Error("foreign key was wrong value", a.BooksID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.BooksID))
		reflect.Indirect(reflect.ValueOf(&a.BooksID)).Set(zero)
		...
		if a.BooksID != x.ID {
			t.Error("foreign key was wrong value", a.BooksID, x.ID)
		}
	}
}
func testBookAuthorToOneSetOpAuthorUsingAuthor(t *testing.T) {
	...
	for i, x := range []*Author{&b, &c} {
		...
		if a.AuthorsID != x.ID {
			t.Error("foreign key was wrong value", a.AuthorsID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.AuthorsID))
		reflect.Indirect(reflect.ValueOf(&a.AuthorsID)).Set(zero)
		...
		if a.AuthorsID != x.ID {
			t.Error("foreign key was wrong value", a.AuthorsID, x.ID)
		}
	}
}
...
var (
	bookAuthorDBTypes = map[string]string{`ID`: `bigint`, `BooksID`: `uuid`, `AuthorsID`: `bigint`}
	_                 = bytes.MinRead
)
